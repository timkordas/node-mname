...
Formats.queryMessage = {
        ...
        question: { type: { format: 'question' }, count: ['header', 'qdCount'] },
        authority: { type: '_nsAuthority', count: ['header', 'nsCount'] }
};
...
var serializers = {
        ...
        '_nsAuthority': {
                encoder: function(v) {
                        assert.arrayOfObject(v, 'authorities');
        ...
        '_nsAdditional': {
                encoder: function(v) {
                        assert.arrayOfObject(v, 'additionals');
        ...
        '_nsData': {
                ...
                decoder: function(buf, pos, parent) {
                        var r, val;
                        ...
                        switch (parent.rtype) {
                        case queryTypes['SOA']:
                                r = decode(buf, 'soa', pos, parent);
                                break;
                        default:
                                throw (new Error('unsupported nsdata type: ' +
                                    queryTypes[parent.rtype]));
                                break;
                        }
...
function decode(raw, format, pos, parent) {
        ...
        for (var f in fmt) {
                ...
                for (var i = 0; i < x; ++i) {
                        if (typeof(type) === 'string') {
                                res = serializers[type].decoder(raw, pos, result);
                        }
                        else if (typeof(type) === 'object') {
                                reftype = type.format;
                                res = decode(raw, reftype, pos, result);
                        }
                        else {
                                throw new TypeError('invalid type');
                        }
