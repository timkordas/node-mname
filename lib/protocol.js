...
var serializers = {
        'UInt32BE': {
                ...
                decoder: function(v, p) {
                        var res = v.readUInt32BE(p);
                        return { val: res, len: 4 };
                }
        },
        'UInt16BE': {
                encoder: function(v, ctx) {
        ...
        '_nsAuthority': {
                encoder: function(v) {
                        ...
                        b = new Buffer(s);
                        for (var n = 0; n < authorities.length; ++n) {
                                authorities[n].copy(b, p);
                                p = p + authorities[n].length;
                        }
                        return b;
                },
                decoder: function(v, p) {
                        return decode(v, 'answer', p);
                }
        },
        '_nsAdditional': {
                encoder: function(v) {
                        assert.arrayOfObject(v, 'additional');
                        var s = 0, p = 0, additional = [];
                        for (var i = 0; i < v.length; ++i) {
                                var r = encode(v[i], 'answer');
                                additional.push(r);
                                s = s + r.length;
                        ...
                        b = new Buffer(s);
                        for (var n = 0; n < additional.length; ++n) {
                                additional[n].copy(b, p);
                                p = p + additional[n].length;
                        }
                        return b;
                },
                decoder: function(v, p) {
        ...
        '_ednsOptions': {
                encoder: function (v) {
                        ...
                        v.forEach(function (opt) {
                                ...
                                head.writeUInt16BE(opt.code, 0);
                                head.writeUInt16BE(opt.data.length, 2);
                                b = Buffer.concat([b, head, opt.data]);
                        });
                        return b;
                },
                decoder: function (buf, pos) {
                        var start = pos;
                        var opts = [];
                        while (pos < buf.length) {
                                var opt = {};
                                opt.code = buf.readUInt16BE(pos);
                                pos += 2;
                                var len = buf.readUInt16BE(pos);
                                pos += 2;
                                opt.data = buf.slice(pos, pos + len);
                                pos += len;
                                opts.push(opt);
                        }

                        return { val: opts, len: (pos - start) };
                }
        },
        '_nsName': {
                encoder: function(v) {
                        ...
                        var b = new Buffer(n.toString().length + 3);
                        var o = 0; //offset

                        for (var i = 0; i < n.length; i++) {
                                var l = n[i].length;
                                assert.ok(l < 64,
                                    v + ': segment "'+n[i]+'" is too long');
                                assert.ok(l > 0,
                                    v + ': segment '+i+' is too short');
                                b[o] = l;
                ...
                decoder: function decodeName(v, p) {
                        var rlen, start = p, name = [];

                        rlen = v.readUInt8(p);
                        while (rlen != 0x00) {
                                p++;
                                /*
                                 * If the top two bits of the rlen are set,
                                 * this is a 'pointer' segment, which refers
                                 * back to a name segment from elsewhere in
                                 * this packet.
                                 */
                                if ((rlen & 0xc0) === 0xc0) {
                                        var ptr = v.readUInt8(p);
                                        ptr = ptr | ((rlen & ~(0xc0)) << 8);
                                        var n = decodeName(v, ptr);
                                        name.push(n.val);
                                        /* ptrs can only be last part */
                                        break;

                                /* Otherwise, assume it's a normal string. */
                                } else {
                                        assert.ok(rlen < 64, 'nsname rlen >64');
        ...
        '_nsData': {
                ...
                decoder: function(buf, pos, ctx) {
                        var r, val;

                        var innerlen = buf.readUInt16BE(pos);
                        pos = pos + 2;
                        var len = 2 + innerlen;

                        switch (ctx.parent.rtype) {
                        case queryTypes['SOA']:
                                r = decode(buf, 'soa', pos, ctx);
                                break;
                        case queryTypes['OPT']:
                                /* opt records are not self-terminating */
                                r = decode(buf.slice(0, pos + innerlen), 
                                    'opt', pos, ctx);
                                break;
                        default:
                        ...
                        assert.ok(innerlen === r.len);
                        return ({val: r.val, len: len});
                }
        },
};

function encode(obj, format, ctx) {
        ...
        for (f in fmt) {
                ...
                if (typeof(type) === 'string') {
                        ...
                        if (type == '_nsData') {
                                res = serializers['_nsData'].
                                    encoder(obj[f], obj['rtype'], kidctx);
                        }
                        else {
                                res = serializers[type].encoder(obj[f], kidctx);
                        }
                }
                else if (typeof(type) === 'object') {
                        reftype = type.format;
                        res = encode(obj[f], reftype, kidctx);
                }
                else {
...
function decode(raw, format, pos, ctx) {
        var size = 0, fmt, field, type, decoder, result = {}, ipos = pos;
        ...
        for (var f in fmt) {
                ...
                var countExpr = fmt[f].count || 1;
                var x = result;
                if (typeof (countExpr) === 'object' &&
                    countExpr instanceof Array) {
                ...
                for (var i = 0; i < x; ++i) {
                        if (typeof(type) === 'string') {
                                res = serializers[type].
                                    decoder(raw, pos, kidctx);
                        }
                        else if (typeof(type) === 'object') {
                                reftype = type.format;
                                res = decode(raw, reftype, pos, kidctx);
                        }
                        else {
                                var err = new TypeError('Invalid format type');
                                err.type = type;
                        ...
                        pos += res.len;
                        if (x > 1)
                                result[f].push(res.val);
                        else
                                result[f] = res.val;
                }
        }

        return {val: result, len: (pos - ipos)};
}
