...
Server.prototype.listen = function listen(port, address, callback, family) {
        assert.number(port, 'port');
        ...
        this._socket.on('error', function onSocketError(err) {
                if (self._socket.bound || family === 'udp4') {
                        self.emit('error', err);
                } else {
                        if (err.code && err.code === 'EINVAL') {
                                ...
                                delete (self._socket);
                                self._socket = null;
                                self.listen(port, address, callback, 'udp4');
                        } else {
                                self.emit('error', err);
                        }
                }
