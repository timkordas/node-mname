...
function parseQuery(raw, src) {
        ...
        if (d.authority) {
                assert.object(d.authority);
                assert.strictEqual(d.authority.rtype, queryTypes['SOA']);
                assert.strictEqual(d.authority.name, d.question.name);
                res.ixfrSoa = d.authority.rdata;
        }
